rowdistr<-
function (crosstablist, comp = "basic", conf.level = 0.95) 
{
    ...
    if (is.matrix(crosstablist) & !is.list(crosstablist)) {
        crosstablist <- list(row.props = sweep(crosstablist, 
            ...
            2, apply(crosstablist, 2, sum), "/"), whole.props = crosstablist/sum(apply(crosstablist, 
            2, sum)), Totals = rbind(cbind(crosstablist, apply(crosstablist, 
            1, sum)), apply(cbind(crosstablist, apply(crosstablist, 
            1, sum)), 2, sum)))
        ...
        if (is.null(names(dimnames(crosstablist$Totals)))) 
            names(dimnames(crosstablist$Totals)) <- c("fac1", 
                "fac2")
    }
    print("Row Proportions", quote = FALSE)
    r1 <- "Row Proportions"
    propmat <- crosstablist$row.props
    n <- nrow(propmat)
    if (comp == "basic" | comp == "within") 
        par(mfrow = c(n, 1), mar = c(1.5, 1.5, 1.5, 1.5), oma = c(3, 
            3, 4, 3))
    ...
    for (j in 1:ncol(p)) {
        for (i1 in 1:(n - 1)) {
            for (i2 in 2:n) {
                temp <- p[i1, j] - p[i2, j] + abs(qnorm((1 - 
                  conf.level)/(2*choose(n,2)))) * c(-1, 1) * sqrt(p[i1, j] * 
                  (1 - p[i1, j])/ns[i1] + p[i2, j] * (1 - p[i2, 
                  j])/ns[i2])
                temp <- round(temp, 3)
    ...
    for (k in 1:nrow(p)) {
        for (i1 in 1:(ncol(p) - 1)) {
            for (i2 in 2:ncol(p)) {
                tempw <- p[k, i1] - p[k, i2] + abs(qnorm((1 - 
                  conf.level)/(2*choose(ncol(p),2))))* c(-1, 1) * sqrt(((p[k, i1] + 
                  p[k, i2]) - ((p[k, i1] - p[k, i2])^2))/ns[k])
                tempw <- round(tempw, 3)
