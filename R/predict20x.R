predict20x<-
function(lmfit,newdata,cilevel = 0.95, digit = 3, print.out = TRUE){
...
# in fitting the above model (note: the variable names do not need to be specified)
	...
	name.row<-1:nrow(newdata)
        #name.col<-attr(lmfit$terms,"term.labels")
	x<-attr(lmfit$terms,"term.labels")
    ...
    pred <- predict.lm(lmfit, newdata, se = TRUE)
    Predicted <- pred$fit
